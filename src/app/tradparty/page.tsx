...
export default async function TradPartyPage({ searchParams }: { searchParams: { token?: string } }) {
  const token = searchParams.token || "";
  if (!token) {
    return (
      <main style={{ padding: 24 }}>
        <h1>Traditional Party / Disco Tech Invitation</h1>
        <p>Missing token.</p>
      </main>
    );
  }
    ...
    return (
      <main style={{ 
        ...
        <div style={{ 
          ...
          {/* RSVP Buttons */}
          <InviteClient token={token} guestName={guest.Name} eventType="tradparty" />
        </div>
      </main>
    );
  } catch (e) {
